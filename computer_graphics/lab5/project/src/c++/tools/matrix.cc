...
Matrix<T>::Matrix(size_t n_row, size_t m_column) : n(n_row), m(m_column) {
  ...
  for (size_t i = 0; i < n; ++i) a[i] = (T*)malloc(m * sizeof(T));
...
Matrix<T>::Matrix(const Matrix& clonner) : n(clonner.n), m(clonner.m) {
  ...
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < m; ++j) a[i][j] = clonner.a[i][j];
}
...
T Matrix<T>::get(size_t i, size_t j) {
  if (/*i < 0 || */ i >= n) throw printOutOfBound("i", i);
  if (/*j < 0 || */ j >= m) throw printOutOfBound("j", j);
  return a[i][j];
}
...
Matrix<T> Matrix<T>::add(const Matrix& one, const Matrix& two) {
  ...
  for (size_t i = 0; i < res.n; ++i)
    for (size_t j = 0; j < res.m; ++j) res.a[i][j] = one.a[i][j] + two.a[i][j];
  return res;
}
...
string Matrix<T>::printOutOfBound(string ij, size_t ij_val) {
  return "Out of bounds: n_row = " + to_string(n) +
         ", m_column = " + to_string(m) + ", and " + ij + " = " +
         to_string(ij_val) + ". ";
}
...
Matrix<T> Matrix<T>::inverse() const {
  ...
  if (check == false) throw "Cannot inverse this matrix. ";
  return res;
}
...
bool matrix_inverse(double** matrix, double** result, const int size) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    for (int j = 0; j < size; ++j) copy[i][j] = matrix[i][j];
  }
  ...
  for (int k = 0; k < size; ++k) {
    ...
    if (fabs(copy[k][k]) < 1e-8) {
      ...
      if (!changed) {
        // Чистим память
        for (int i = 0; i < size; ++i) delete[] copy[i];
