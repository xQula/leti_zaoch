 ...
 * @throws None.
 */
void bank::BankAccount::populate(const double balance) {
    accNo = add_id();
    accBal = balance;
...
auto bank::BankAccount::withdraw(const double amount) -> void {
     accBal -= amount;
}

auto bank::BankAccount::deposit(const double amount) -> void {
    accBal += amount;
}
