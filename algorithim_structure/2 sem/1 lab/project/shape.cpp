...
#include "screen.h"
#include "shape.h"
using namespace std; // Пример добавки: дополнительный фрагмент - полуокружность
class h_circle:public rectangle, public reflectable {
    bool reflected;
public: h_circle(point a, point b, bool r=true) : rectangle(a, b), reflected(r) { }
...
void h_circle::draw() {
    ...
    int radius = (ne.xo - sw.xo) / 2; int x = 0, y = radius, delta = 2 - 2 * radius, error = 0;
    while (y >= 0) { // Цикл рисования
         if (reflected) { put_point(x0 + x, y0 + y * 0.7);
             put_point(x0 - x, y0 + y * 0.7); }
         else { put_point(x0 + x, y0 - y*0.7);
             put_point(x0 - x, y0 - y*0.7); } error = 2 * (delta + y) - 1;
...
class face: public rectangle {
    ...
    w(neast().xo - swest().xo + 1), // Инициализация данных
    h(neast().yo - swest().yo + 1), // строго в порядке объявления!
    l_eye(point(swest().xo + 2, swest().yo + h * 3 / 4), 2), r_eye(point(swest().xo + w - 4, swest().yo + h * 3 / 4), 2), mouth(point(swest().xo + 2, swest().yo + h / 4), w - 4) { }
    void face::draw() { rectangle::draw();
    ...
    class rectangleWithPlus: public rectangle { int w, h; line verticalLine; line horizontalLine;
    public: rectangleWithPlus(point, point);
            void draw() override;
            void move(int, int) override; void resize(int) override { }
    };
    rectangleWithPlus::rectangleWithPlus(point a, point b) : rectangle(a, b),
    // Инициализация базового класса
    w(neast().xo - swest().xo + 1), h(neast().yo - swest().yo + 1), verticalLine(point(north().xo, north().yo), point(south().xo, south().yo)), horizontalLine(point(west().xo, west().yo), point(east().xo, east().yo)) { }
    void rectangleWithPlus::draw() { rectangle::draw();
        int a = (swest().xo + neast().xo) / 2;
        int b = (swest().yo + neast().yo) / 2; put_point(point(a, b)); }
        void rectangleWithPlus::move(int a, int b) { rectangle::move(a, b); verticalLine.move(a, b); horizontalLine.move(a, b); }
        ...
        line brim(point(0,15),17); face face(point(15, 10),point(27, 18));
        ...
        shape_refresh();
        cout << endl << "Generated..." << endl << endl;
        //cin.get();
        // 2. Подготовка к сборке
        hat.rotate_right(); brim.resize(2);
        face.resize(2);
        beard.flip_vertically();
        shape_refresh(); cout << endl << "Prepared..." << endl << endl;
        // cin.get();
        ...
        pos3(rectangleWithPlus3, face);
        pos7(rectangleWithPlus7, brim);
