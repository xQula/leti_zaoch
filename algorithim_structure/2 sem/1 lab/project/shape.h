...
void screen_init() { for (auto & y : screen) for (auto &x : y) x = white; }
void screen_destroy() { for (auto & y : screen) for (auto &x : y) x = black; }
bool on_screen(int a, int b) {
    ...
    void put_line(int x0, int y0, int x1, int y1) {
        /* Алгоритм Брезенхэма для прямой: рисование отрезка прямой от (x0,y0) до (x1,y1). Уравнение прямой: b(x-x0) + a(y-y0) = 0.
        * Минимизируется величина abs(eps), где eps = 2*(b(x-x0)) + a(y-y0). */
        int dx = 1; int a = x1 - x0; if (a < 0) dx = -1, a = -a;
        int dy = 1; int b = y1 - y0; if (b < 0) dy = -1, b = -b;
    ...
    void shape_refresh() {
        ...
        public: virtual void rotate_left() = 0;
                    //Повернуть влево
    ...
    protected: point w, e; public: line (point a, point b) : w(a), e(b) { };
    line (point a, int L) : w(point(a.xo + L - 1, a.yo)), e(a) { };
    ...
    void resize(int d) override { e.xo += (e.xo - w.xo) * (d - 1); e.yo += (e.yo - w.yo) * (d - 1); }
    // Увеличение длины линии в (d) раз
    };
    // Прямоугольник
    class rectangle:public rotatable { /* nw ------ n ------ ne | | | | w c e | | | | sw ------ s ------ se */
    protected: point sw, ne;
    public: rectangle(point a, point b) : sw(a), ne(b) { }
    ...
    point neast() const override { return ne; } point seast() const override { return point(ne.xo, sw.yo); }
    point nwest() const override { return point(sw.xo, ne.yo); } point swest() const override { return sw; }
    void rotate_right() override { // Поворот вправо относительно se
         int w = ne.xo - sw.xo, h = ne.yo - sw.yo;
         // (учитывается масштаб по осям)
         sw.xo = ne.xo - h * 2; ne.yo = sw.yo + w / 2; }
         void rotate_left() override { // Поворот влево относительно sw//
        int w = ne.xo - sw.xo, h = ne.yo - sw.yo; ne.xo = sw.xo + h * 2;
        ne.yo = sw.yo + w / 2;
    }
        void move(int a, int b) override { sw.xo += a; sw.yo += b; ne.xo += a; ne.yo += b; }
        void resize(int d) override { ne.xo += (ne.xo - sw.xo) * (d - 1);
        ne.yo += (ne.yo - sw.yo) * (d - 1); }
        void draw() override { put_line(nwest(), ne); put_line(ne, seast( )); put_line(seast(), sw); put_line(sw, nwest()); }
    };
    void up(shape& p, const shape& q) { // поместить p над q // Это ОБЫЧНАЯ функция, не член класса! Динамическое связывание!!
        point n = q.north(); point s = p.south(); p.move(n.xo - s.xo, n.yo - s.yo + 1); }
        void pos2(shape& p, const shape& q) { point n = q.nwest(); point s = p.swest(); p.move(n.xo + s.xo - 9, n.yo + s.yo - 12); }
        void pos3(shape& p, const shape& q) { point n = q.east(); point s = p.seast(); p.move(n.xo + s.xo - 7, s.yo + 6); }
        void pos7(shape& p, const shape& q) {
        point n = q.nwest();
        point s = p.swest();
        p.move(s.xo + n.xo-8, n.yo + s.yo);
    }
        void pos8(shape& p, const shape& q) { point n = q.neast(); point s = p.seast(); p.move(n.xo, n.yo + s.yo);
    }
