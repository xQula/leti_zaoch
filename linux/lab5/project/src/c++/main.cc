...
// 2 -- addressing to nullptr operation usage
...
int main(int argc, char *argv[]) {
    const string type_func = argv[1];
    const int type_err = atoi(argv[2]);
    ...
    {
        sigaction(SIGFPE, &sig, nullptr); // SIGFPE = 8, invalid operation (overflow, dividing by 0)
        sigaction(SIGSEGV, &sig, nullptr); // SIGSEGV = 11, memory protection breach
    ...
    switch (type_err) {
        ...
        case 2: // memory protection breach: trying to address to nullptr pointer
        {
            char *c = nullptr;
            *c = 'z';
            break;
...
void ErrorHandler(const int local) {
    switch (local) {
        ...
        {
            puts("Memory protection breach: trying to address to nullptr pointer!");
            exit(2);
        }
